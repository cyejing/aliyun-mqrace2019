...
//        BOUND4_NUM = 703893211;
    ...
    public static void main(String[] args) throws Exception {
        ...
        for(int i = 0; i < sends.length; ++i) {
            sends[i] = new Thread(new DemoTester.Producer(messageStore, msgNum, sendCounter, tIndexCounter));
        ...
        for(i = 0; i < msgChecks.length; ++i) {
            msgChecks[i] = new Thread(new DemoTester.MessageChecker(messageCheckNum));
        ...
        for(i = 0; i < msgChecks.length; ++i) {
            msgResultQueue.add(new DemoTester.OneResult(-1, 0L, 0L, 0L, 0L, (List)null));
        }
        ...
        for(i = 0; i < valueChecks.length; ++i) {
            valueChecks[i] = new Thread(new DemoTester.ValueChecker(valueCheckNum));
        ...
        for(i = 0; i < valueChecks.length; ++i) {
            valueResultQueue.add(new DemoTester.OneResult(-1, 0L, 0L, 0L, 0L, (List)null));
        }
    ...
    private static void rangerPicker(MessageStore messageStore, int getMsgNum, int maxCheckTime, long msgCheckStart, boolean pickMessageOrValue) throws InterruptedException {
        ...
        for(i = 0; i < fewThreads; ++i) {
            getMessageThreads[i] = new Thread(new DemoTester.RangePicker(messageStore, startNum, BOUND_1, maxMsgCheckTimes, BOUND1_NUM / 10, pickMessageOrValue, false));
        ...
        for(i = 0; i < getMessageThreads.length; ++i) {
            getMessageThreads[i] = new Thread(new DemoTester.RangePicker(messageStore, BOUND_1 + 1, BOUND_2, maxMsgCheckTimes, bound2CheckNum, pickMessageOrValue, false));
            getMessageThreads[i].setName("RangePicker-" + tNum++);
        }
        ...
        for(i = 0; i < fewThreads; ++i) {
            getMessageThreads[i] = new Thread(new DemoTester.RangePicker(messageStore, BOUND_1 + 1, BOUND_2, maxMsgCheckTimes, bound2CheckNum, pickMessageOrValue, true));
            getMessageThreads[i].setName("RangePicker-" + tNum++);
        }
        ...
        for(i = 0; i < getMessageThreads.length; ++i) {
            getMessageThreads[i] = new Thread(new DemoTester.RangePicker(messageStore, BOUND_2 + 1, BOUND_3, maxMsgCheckTimes, bound2CheckNum, pickMessageOrValue, false));
            getMessageThreads[i].setName("RangePicker-" + tNum++);
        }
        ...
        for(i = 0; i < getMessageThreads.length; ++i) {
            getMessageThreads[i] = new Thread(new DemoTester.RangePicker(messageStore, BOUND_3 + 1, BOUND_4, maxMsgCheckTimes, bound2CheckNum, pickMessageOrValue, false));
            getMessageThreads[i].setName("RangePicker-" + tNum++);
        }
    ...
    private static boolean isInARange(DemoTester.OneResult rst, Message msg) {
        return msg.getA() >= rst.aMin && msg.getA() <= rst.aMax;
    }

    private static void printStatus(DemoTester.OneResult rst) {
    ...
    static class ValueChecker implements Runnable {
        ...
        public ValueChecker(AtomicLong numCounter) {
        ...
        private void computeRealValue(DemoTester.OneResult rst, int repeatTimes, int mask, boolean needTotal, long s) {
        ...
        public void run() {
            DemoTester.OneResult rst = null;

            while(true) {
                while(true) {
                    try {
                        rst = (DemoTester.OneResult) DemoTester.valueResultQueue.take();
                        if (rst.bound == -1) {
                            return;
                        }
    ...
    static class MessageChecker implements Runnable {
        ...
        public MessageChecker(AtomicLong numCounter) {
        ...
        private void checkError(String reason, DemoTester.OneResult rst) {
        ...
        private void checkResultSpec(DemoTester.OneResult rst) {
            int idx = 0;
            int mask = -1;
            int repeat = -1;
            int mask2 = -1;
        ...
        public void run() {
            ...
            while(true) {
                while(true) {
                    try {
                        rst = (DemoTester.OneResult) DemoTester.msgResultQueue.take();
                        if (rst.bound == -1) {
                            return;
                        }
    ...
    private static class RangePicker implements Runnable {
        ...
        private void getResult(long tMin, long tMax, long aMin, long aMax) {
            if (this.pickMessageOrValue) {
                ...
                DemoTester.msgResultQueue.add(new DemoTester.OneResult(this.checkMax, tMin, tMax, aMin, aMax, msgs));
            } else {
                long value = this.messageStore.getAvgValue(aMin, aMax, tMin, tMax);
                DemoTester.valueResultQueue.add(new DemoTester.OneResult(this.checkMax, tMin, tMax, aMin, aMax, value));
            }
